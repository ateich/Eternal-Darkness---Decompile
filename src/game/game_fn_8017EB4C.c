...
extern u32 lbl_80651CC0;
extern u16 lbl_80651CC4;
...
void fn_8017EB4C(u8* object, ShortCoord3* first, ShortCoord3* second,
                 u8* config)
{
    ShortCoord3 base;
    ShortCoord3 current;
    ...
    *(u32*)&zero = lbl_80651CC0;
    *(u16*)((u8*)&zero + 4) = lbl_80651CC4;
    count = config[0];
    ...
    *(void**)(object + 0x68) = lbl_8064D224;
    ...
    for (i = 0; (u8)i < count; i++) {
        ...
        fn_8018F014(&work, 4);
        fn_80180554(entry, &current, &work, &zero,
                    *(u16*)(config + 8) + (fn_800FBFB0() & 0xF), 1);

        random_value = config[1];
        random_value = random_value - ((random_value - 1) & fn_800FBFB0());
        fn_801805E0(entry + 0x20, 4, random_value, (i & 0x3F) * 4,
                    config + 0x1D, lbl_80650938);

        if (*(int*)(config + 0x18) != 0) {
            current.x += base.x;
            current.y += base.y;
            current.z += base.z;
        }
