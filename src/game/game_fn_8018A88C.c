...
extern void fn_8018A310(void*, void*, void*, int);
...
void fn_8018A88C(u8* object)
{
    ...
    if (count > 3) {
        ...
        fn_8018A310(*(void**)(object + 0x4C), lbl_80607900, vertex_data, count);
    } else {
        int byte_offset;
        int i;
        ...
        byte_offset = 0;
        for (; i < count; i++) {
            fn_80188A7C(object_data, vertex_data + byte_offset,
                        fn_8018D020);
            {
                int j;
                for (j = 0; j < object_data[0x20]; j++) {
